...
The main supercomputer comprises 32 `gpu` nodes, each with 4 Tesla V100 32G gpus, and 4 `infer` nodes with 4 Tesla T4 16G each, with CPUs built on the IBM PowerPC architecture. Bede also includes a NVIDIA Grace-Hopper partition, with 7 nodes with a Grace Hopper Superchip with a H100 96GB gpu module, and one node with 2 Grace Hopper Superchips with a H100 144GB gpu module each, with the CPU modules built on ARM.

Within each node in the original system, the CPU and GPUs are linked to a single NVLink 2.0 bus. Nodes are connected with dual-rail Mellanox EDR Infiniband interconnects. The Grace Hopper Superchips include a 900 GB/s NVLink-C2C within the chip and Mellanox CONNECTX-7 NDR200 Infiniband interconnects between nodes.
...
For researchers on a project registered on Bede to gain access, you will need to create an EPCC SAFE account at <https://safe.epcc.ed.ac.uk/>, and then request access with the project code which you should be able to get from your project PI. Further details on how to gain access to Bede are at <https://bede-documentation.readthedocs.io/en/latest/usage>
